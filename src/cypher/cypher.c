...
static int cross_join_nodes(binding_t **bindings, int *bind_count, cbm_node_t *extra_nodes,
    ...
    if (cbm_cypher_cross_join_alloc(*bind_count, extra_count, opt, &alloc_n) != 0) {
        if (err) {
            *err = heap_strdup("Query aborted: Cartesian product exceeds memory safety limits. Use narrower filters.");
        }
        return -1; /* product overflows int/size_t: fail the query */
    }
    binding_t *new_bindings = malloc(alloc_n * sizeof(binding_t));
    if (!new_bindings) {
        if (err) {
            *err = heap_strdup("Query aborted: Out of memory.");
        }
        return -1; /* OOM: propagate, don't silently skip the MATCH */
    }
    size_t new_count = 0;
    for (int bi = 0; bi < *bind_count; bi++) {
...
static int execute_single(cbm_store_t *store, cbm_query_t *q, const char *project, int max_rows,
    ...
    while (pipe->vtable->next(pipe, &b)) {
        /* #601: 스트리밍 방식으로 결과를 가공 수집하는 동안 전체 실행 데드라인 예산(Budget)을 점검합니다. */
        if ((bind_count & CYPHER_DEADLINE_CHECK_MASK) == 0 && cypher_deadline_exceeded()) {
            binding_free(&b);
            break;
        }
